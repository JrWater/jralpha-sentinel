...
1. **Trend Vector (45%)** — 0–2 DTE credit spreads in the regime's direction — the 250-session backtest measured 87% wins for credit on this signal (vs 40% for debit verticals), and a one-week window rewards win probability; debit verticals remain the fallback. Longs only in risk_on, shorts only in risk_off.

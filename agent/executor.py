...
class Executor:
    ...
    def submit(self, proposal: Proposal, closing: bool = False, *,
        ...
        if closing:
            # Exits reduce exposure and may be generated by orphan recovery;
            # they are constrained by the global declared wire vocabulary but
            # do not require an entry strategy capability.
            ...
            # Entries must use a shape this strategy owns, not merely one
            # another strategy happened to declare globally.
            shape = self.manifest.find_shape_for_strategy(
                proposal.engine, order_class=proposal.order_class,
                type=proposal.type, time_in_force=proposal.time_in_force,
                legs=len(proposal.legs))
        if shape is None:
            raise RuntimeError(
                f"undeclared shape for strategy {proposal.engine} refused "
                f"before submission: "
